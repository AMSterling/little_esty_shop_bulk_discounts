...
RSpec.describe 'merchant bulk discount index' do
  it 'lists all discounts from the merchant' do
    ...
    within("#discount-#{discount1.id}") do
      ...
      expect(page).to have_link('Manage 10% Discount')
      expect(page).to_not have_content('Percentage: 15%')
      expect(page).to_not have_content('Quantitu Threshold: 20')
      expect(page).to_not have_content('Percentage: 20%')
      expect(page).to_not have_content('Quantitu Threshold: 30')
    end
    ...
    within("#discount-#{discount2.id}") do
      ...
      expect(page).to have_content('Status: disabled')
      expect(page).to have_link('Manage 15% Discount')
    ...
    within("#discount-#{discount3.id}") do
      ...
      expect(page).to have_content('Status: disabled')
      expect(page).to have_link('Manage 20% Discount')
      expect(page).to_not have_content('Percentage: 10%')
      expect(page).to_not have_content('Quantity Threshold: 15')
      expect(page).to_not have_content('Percentage: 15%')
      expect(page).to_not have_content('Quantitu Threshold: 20')
